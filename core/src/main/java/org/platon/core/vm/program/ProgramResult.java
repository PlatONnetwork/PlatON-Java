...
import org.platon.core.vm.CallCreate;
...
public class ProgramResult {
    ...
    public void spendEnergon(long energon) {
        energonUsed += energon;
    }
    ...
    public void refundEnergon(long energon) {
        energonUsed -= energon;
    }
    ...
    public void addCallCreate(byte[] data, byte[] destination, byte[] energonLimit, byte[] value) {
        getCallCreateList().add(new CallCreate(data, destination, energonLimit, value));
    }
    ...
    public InternalTransaction addInternalTransaction(byte[] parentHash, int deep,byte[] sendAddress,String note,TransactionType type, byte[] value, byte[] receiveAddress,
                                                      long referenceBlockNum, byte[] referenceBlockHash, DataWord energonPrice,
                                                      DataWord energonLimit, byte[] data) {
        InternalTransaction transaction = new InternalTransaction(parentHash,deep,size(internalTransactions),sendAddress,note,type,value,receiveAddress,referenceBlockNum,referenceBlockHash,energonPrice.getData(),energonLimit.getData(),data);
    ...
    public void addFutureRefund(long energonValue) {
        futureRefund += energonValue;
    }
