...
import org.platon.storage.datasource.AbstractCachedSource;
import org.platon.storage.datasource.AsyncFlushable;
...
public class DbFlushManager {
    ...
    public DbFlushManager(CoreConfig config, Set<DbSource> dbSources, AbstractCachedSource<byte[], byte[]> stateDbCache) {
        this.config = config;
        this.dbSources = dbSources;
        sizeThreshold = 64 * 1024 * 1024;
        commitsCountThreshold = 0;
        flushAfterSyncDone = true ;
        this.stateDbCache = stateDbCache;
    }

    @Autowired
    public void setEthereumListener(CompoplexPlatonListener listener) {
        if (!flushAfterSyncDone) return;
        /*listener.addListener(new PlatonListenerAdapter() {
            @Override
