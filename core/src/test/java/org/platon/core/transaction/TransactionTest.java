...
import org.platon.core.transaction.proto.ContractDeployRequest;
import org.platon.core.transaction.proto.TransactionType;
import org.platon.core.transaction.proto.defaultRequestData;
...
public class TransactionTest {
    ...
    @Test /* build a transaction, sign it and send(using pb), and decode it and verify */
    public void testTransactionFlow() {
        ...
        ECKey ecKey = ECKey.fromPrivate(cowPrivKey);

        byte[] dataBase = Hex.decode("a9e880872386f26fc1000085e8d4a510008203e89413978aee95f38490e9769c39b2773ed763d9cd5f80");
        defaultRequestData.Builder defReqData = defaultRequestData.newBuilder();
        defReqData.setData(ByteString.copyFrom(dataBase));
        //Any any = Any.pack(defReqData.build());
        byte[] data = defReqData.build().toByteArray();
        //byte[] data = any.toByteArray();
        TransactionType transactionType = TransactionType.TRANSACTION;
        BigInteger value = new BigInteger("23");
        byte[] receiveAddress = Hex.decode("f82804881bc16d674ec8000094cd2a3d9f938e13");
        long referenceBlockNum = 101234L;
        byte[] referenceBlockHash =  HashUtil.sha3(receiveAddress); //
        BigInteger energonPrice = new BigInteger("1000000");
        BigInteger energonLimit = new BigInteger("300");
        Integer chainId = 28;

        // next need to read from a file, and do the test list

        Transaction tx = new Transaction(transactionType, value, receiveAddress, referenceBlockNum,
                referenceBlockHash, energonPrice, energonLimit, data, chainId);

        tx.sign(ecKey);

        byte[] txEncode = tx.getEncoded();
        Transaction newTx = new Transaction(txEncode);
        Assert.assertTrue(transactionType.ordinal() == newTx.getTransactionType().ordinal());
        Assert.assertTrue(value.compareTo(newTx.getValue()) == 0);
        Assert.assertArrayEquals(receiveAddress, newTx.getReceiveAddress());
        Assert.assertTrue(referenceBlockNum == newTx.getReferenceBlockNum());
        Assert.assertArrayEquals(referenceBlockHash, newTx.getReferenceBlockHash());
        Assert.assertTrue(energonPrice.compareTo(newTx.getEnergonPrice()) == 0);
        Assert.assertTrue(energonLimit.compareTo(newTx.getEnergonLimit()) == 0);
        Assert.assertArrayEquals(tx.getData(), newTx.getData());

        //todo ignore this chainID testify until there is a work rule to generate chainId
//        if (chainId != null)
//            Assert.assertTrue(chainId == newTx.getChainId());

        Assert.assertArrayEquals(tx.getRawHash(), newTx.getRawHash());
        Assert.assertArrayEquals(tx.getHash(), newTx.getHash());
        Assert.assertArrayEquals(tx.getSender(), newTx.getSender());

        tx.verify();
        newTx.verify();

        Assert.assertTrue(tx.equals(newTx));
    }

    @Test
    public void testTransaction1() {
        byte[] privKey = Hex.decode("c85ef7d79691fe79573b1a7064c19c1a9819ebdbd1faaab1a8ec92344438aaf4");
        byte[] data = null;
        TransactionType transactionType = TransactionType.TRANSACTION;
        BigInteger value = BigInteger.ZERO;
        byte[] receiveAddress = null;
        long referenceBlockNum = 1000;
        byte[] referenceBlockHash = HashUtil.sha3(privKey);
        BigInteger energonPrice = new BigInteger("20000");
        BigInteger energonLimit = new BigInteger("100");
        Integer chainId = null;

        testTransactionbase(privKey, data, transactionType,
                value, receiveAddress, referenceBlockNum,referenceBlockHash,
                energonPrice, energonLimit, chainId);
    }

    @Test
    public void testTransaction1_1() {
        byte[] privKey = Hex.decode("c85ef7d79691fe79573b1a7064c19c1a9819ebdbd1faaab1a8ec92344438aaf4");
        byte[] data = null;
        TransactionType transactionType = TransactionType.CONTRACT_DEPLOY;
        BigInteger value = BigInteger.ZERO;
        byte[] receiveAddress = new byte[0];
        long referenceBlockNum = 1000;
        byte[] referenceBlockHash = HashUtil.sha3(privKey);
        BigInteger energonPrice = new BigInteger("20000");
        BigInteger energonLimit = new BigInteger("100");
    ...
    public void testTransaction3() {
        ...
        String classStr = "mySmartContract.class";
        byte[] className = classStr.getBytes();
        String codeBin = "79573b1a7064c19c1a9819hy79573b1a7064c19c1a981979573b1a7064c19c1a981979573b1a7064c19c1a9819";
        byte[] codeBinary = codeBin.getBytes();
        ContractDeployRequest.Builder newContract = ContractDeployRequest.newBuilder();
        newContract.setClassName(classStr);
        newContract.setData(ByteString.copyFrom(codeBinary));
        byte[] data = newContract.build().toByteArray();
    ...
    public void testTransaction4() {
        ...
        byte[] receiveAddress = new byte[0];
        long referenceBlockNum = 1000;
        byte[] referenceBlockHash = HashUtil.EMPTY_HASH;
        BigInteger energonPrice = new BigInteger("20000");
        BigInteger energonLimit = new BigInteger("100");
        //Integer chainId = null;

        testTransactionbase(privKey, null, transactionType,
                value, receiveAddress, referenceBlockNum, referenceBlockHash,
                energonPrice, energonLimit, null);
    }
...
//        BigInteger value;
//        byte[] receiveAddress;
//        long referenceBlockNum;
//        byte[] referenceBlockHash;
//        BigInteger energonPrice;
//        BigInteger energonLimit;
//        Integer chainId;
//
//        testTransactionbase(privKey, data, transactionType,
//                value, receiveAddress, referenceBlockNum,referenceBlockHash,
...
//    }
    ...
    /* build a transaction, sign it and send(using pb), and decode it and verify */
    public void testTransactionbase(byte[] privKey, byte[] data, TransactionType transactionType,
                                    BigInteger value, byte[] receiveAddress, long referenceBlockNum, byte[] referenceBlockHash,
                                    BigInteger energonPrice, BigInteger energonLimit, Integer chainId) {
//        byte[] privKey = Hex.decode("c85ef7d79691fe79573b1a7064c19c1a9819ebdbd1faaab1a8ec92344438aaf4");
//        ECKey ecKey = ECKey.fromPrivate(privKey);
//
//        byte[] data = Hex.decode("a9e880872386f26fc1000085e8d4a510008203e89413978aee95f38490e9769c39b2773ed763d9cd5f80");
//        TransactionType transactionType = TransactionType.TRANSACTION;
//        BigInteger value = new BigInteger("23");
//        byte[] receiveAddress = Hex.decode("f82804881bc16d674ec8000094cd2a3d9f938e13");
//        long referenceBlockNum = 101234L;
...
//        Integer chainId = 35;
        ...
        Assert.assertTrue(transactionType.ordinal() == newTx.getTransactionType().ordinal());
        Assert.assertTrue(value.compareTo(newTx.getValue()) == 0);
        if(receiveAddress != null && newTx.getReceiveAddress() != null) {
            Assert.assertArrayEquals(receiveAddress, newTx.getReceiveAddress());
        }
        Assert.assertTrue(referenceBlockNum == newTx.getReferenceBlockNum());
        Assert.assertArrayEquals(referenceBlockHash, newTx.getReferenceBlockHash());
        Assert.assertTrue(energonPrice.compareTo(newTx.getEnergonPrice()) == 0);
        Assert.assertTrue(energonLimit.compareTo(newTx.getEnergonLimit()) == 0);
        if(data != null && newTx.getData() != null) {
            Assert.assertArrayEquals(data, newTx.getData());
        }
        if (chainId != null)
            Assert.assertTrue(chainId == newTx.getChainId());
        if(transactionType == TransactionType.CONTRACT_DEPLOY)
            Assert.assertArrayEquals(tx.getContractAddress(), newTx.getContractAddress());
    ...
    public void test1() throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, IOException {
        ...
        String signature = Base64.toBase64String(key.doSign(txHash).toByteArray());
    ...
    public void test2() throws Exception {
        ...
        BigInteger gasPrice = new BigInteger("09184e72a000");
        BigInteger gas = new BigInteger("4255");
        long blockNum = 1000000000L;
        byte[] blockHash = Hex.decode("1234567890abcdef1234567890abcdef");

        // Tn (nonce); Tp(pgas); Tg(gaslimi); Tt(value); Tv(value); Ti(sender);  Tw; Tr; Ts
        Transaction tx = new Transaction(TransactionType.TRANSACTION, value, ecKey.getAddress(),
                blockNum, blockHash, gasPrice, gas, null);
