...
import org.platon.common.exceptions.DataDecodingException;
import org.platon.common.exceptions.DataEncodingException;
...
public final class Numeric {
    ...
    public static String encodeQuantity(BigInteger value) {
        if (value.signum() != -1) {
            ...
            throw new DataEncodingException("Negative values are not supported.");
        }
    }

    public static BigInteger decodeQuantity(String value) {
        if (!isValidHexQuantity(value)) {
            throw new DataDecodingException("Value must be in format 0x[1-9]+[0-9]* or 0x0");
        }
        try {
            return new BigInteger(value.substring(2), 16);
        } catch (NumberFormatException e) {
            throw new DataDecodingException("Negative, invalid data format ", e);
        }
    }
    ...
    public static boolean containsHexPrefix(String input) {
        return !StringEnhance.isEmpty(input) && input.length() > 1
    ...
    public static String toHexStringWithPrefixSafe(BigInteger value) {
        ...
        if (result.length() < 2) {
            result = StringEnhance.zeros(1) + result;
        }
    ...
    private static String toHexStringZeroPadded(BigInteger value, int size, boolean withPrefix) {
        ...
        if (length < size) {
            result = StringEnhance.zeros(size - length) + result;
        }
