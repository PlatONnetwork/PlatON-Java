...
//        private final static ECKey EC_KEY = new ECKey();
...
//            Mockito.when(platon.getBalance(any())).thenReturn(RETURN_BALANCE);
//
//            Mockito.when(platon.getBestBlock()).thenReturn(createBlock());
//            Mockito.when(platon.getBlockByHash(any(byte[].class))).thenReturn(createBlock());
//            Mockito.when(platon.getBlockByNumber(any(Long.class))).thenReturn(createBlock());
//            Mockito.when(platon.getTransactionInfo(any(byte[].class))).thenReturn(createTxInfo());
//            Mockito.when(platon.submitTransaction(any())).thenReturn(CompletableFuture.completedFuture(createTx()));
//        }
//
...
//            block.sealBlock(createBlockHeader().toByteArray());
//            return block;
//        }
//
...
//            bodyBuild.setType(TransactionType.CONTRACT_CALL);
//            bodyBuild.setValue(ByteString.copyFrom(BigInteger.ZERO.toByteArray()));
...
//            bodyBuild.setEnergonPrice(ByteString.copyFrom(BigInteger.valueOf(10000).toByteArray()));
...
//            baseBuild.setSignature(new String(WalletUtil.sign(messageHash,EC_KEY)));
...
//
//            // - atpGetBalance
//            String addr = "0xa3ae7747a0690701cc84b453524fa7c99afcd8ac";
...
//
//            // - atpGetBlockByNumber
//            //blockResponse = protoRpc.atpGetBlockByNumber(Numeric.toHexStringWithPrefix(BigInteger.valueOf(RETURN_BLOCK_NUM)), true);
//            //Assert.assertNotNull(blockResponse);
//
...
//        }
//
//        public void testAtpAccounts() throws Exception {
//
//            // 生成几个文件
//            for (int i = 0; i < 5; i++) {
//                keystore.storeKey(new ECKey(), "1111" + i);
//            }
//
//            // store wallet file
//            StringArrayResponse response = protoRpc.atpAccounts();
//            List<ByteString> byteStringList = response.getDataList().asByteStringList();
//            for (int i = 0; i < byteStringList.size(); i++) {
//                System.out.println(Numeric.toHexString(byteStringList.get(i).toByteArray()));
//            }
//        }
//    }
//
//    @Test
//    public void testTestConig(){
//        System.out.println("Start test config.");
//        PlatonImpl platon = (PlatonImpl) PlatonFactory.createPlaton(TestConfig.class);
//        Keystore keystore = platon.getApplicationContext().getBean(Keystore.class);
//        Assert.assertNotNull(keystore);
//    }
//
//    @Test
//    public void testMain() throws Exception {
//        System.out.println("Start Platonj...");
//        PlatonImpl platon = (PlatonImpl) PlatonFactory.createPlaton(TestConfig.class);
//        TestRunner test = platon.getApplicationContext().getBean(TestRunner.class);
//        //test.testAtpAccounts();
//        test.runTest();
//    }
//
//    @Test
