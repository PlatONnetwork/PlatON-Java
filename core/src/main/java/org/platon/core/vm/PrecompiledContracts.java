...
import org.platon.common.utils.BIUtil;
...
import org.platon.crypto.zksnark.*;

import java.math.BigInteger;

import static org.platon.common.utils.BIUtil.*;
import static org.platon.common.utils.ByteUtil.*;
...
public class PrecompiledContracts {
    ...
    public static abstract class PrecompiledContract {
        public abstract long getEnergonForData(byte[] data);
    ...
    public static class Identity extends PrecompiledContract {
        ...
        @Override
        public long getEnergonForData(byte[] data) {

            // Energon charge for the execution:
    ...
    public static class Sha256 extends PrecompiledContract {
        ...
        public Pair<Boolean, byte[]> execute(byte[] data) {
            ...
            if (data == null) return Pair.of(true, HashUtil.sha3(EMPTY_BYTE_ARRAY));
            return Pair.of(true, HashUtil.sha3(data));
        }
    }
        ...
        public Pair<Boolean, byte[]> execute(byte[] data) {
            ...
            try {
                ...
                if (validateV(v) && signature.validateComponents()) {
                    out = DataWord.of(WalletUtil.signatureToAddress(h, signature));
                }
            } catch (Throwable any) {
            }
    ...
    public static class ModExp extends PrecompiledContract {
        ...
        public long getEnergonForData(byte[] data) {
            ...
            BigInteger energon = BigInteger.valueOf(multComplexity)
                    .multiply(BigInteger.valueOf(Math.max(adjExpLen, 1)))
                    .divide(GQUAD_DIVISOR);

            return isLessThan(energon, BigInteger.valueOf(Long.MAX_VALUE)) ? energon.longValue() : Long.MAX_VALUE;
        }
