...
public class ByteUtil {
    ...
    /**
     * construct the key stored in the extradb
     *
     * @param hash hash of block
     * @param tail identifier
     * @return the key
     */
    public static byte[] toSlice(byte[] hash, byte tail) {
        byte[] ret = new byte[hash.length + 1];
        System.arraycopy(hash, 0, ret, 0, hash.length);
        ret[hash.length] = tail;
        return ret;
    }
    ...
    /**
     * check if the array contains the byte[]
     *
     * @param array the array
     * @param key   the key
     * @return true or false
     */
    public static boolean contains(Collection<byte[]> array, byte[] key) {
