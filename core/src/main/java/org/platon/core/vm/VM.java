...
import org.platon.core.vm.program.Stack;
...
import static org.platon.core.vm.OpCode.*;
 ...
 * through a parameter, Energon, which limits the total amount of computation done.
 *
 ...
 * state changes intact is the out-of-Energon (OOG) exception.
 *
...
public class VM {
    ...
    private static final String logString = "{}    Op: [{}]  Energon: [{}] Deep: [{}]  Hint: [{}]";
...
//        put(SAR, BlockchainConfig::eip145);
    ...
    public VM() {
        this(CoreConfig.getInstance());
    }

    @Autowired
    public VM(CoreConfig config) {
        this.config = config;
        vmTrace = config.vmTrace();
        dumpBlock = config.dumpBlock();
    }

    private long calcMemEnergon(EnergonCost energonCosts, long oldMemSize, BigInteger newMemSize, long copySize) {
        long EnergonCost = 0;

        // Avoid overflows
        if (newMemSize.compareTo(MAX_MEM_SIZE) > 0) {
            throw Program.Exception.EnergonOverflow(newMemSize, MAX_MEM_SIZE);
        }
        ...
        if (memoryUsage > oldMemSize) {
            ...
            long memEnergon = (energonCosts.getMEMORY() * memWords + memWords * memWords / 512)
                    - (energonCosts.getMEMORY() * memWordsOld + memWordsOld * memWordsOld / 512);
            EnergonCost += memEnergon;
        }

        if (copySize > 0) {
            long copyEnergon = energonCosts.getCOPY_ENERGON() * ((copySize + 31) / 32);
            EnergonCost += copyEnergon;
        }
        return EnergonCost;
    }

    private boolean isDeadAccount(Program program, byte[] addr) {
        return !program.getStorage().isExist(addr) || program.getStorage().getAccount(addr).isEmpty();
    }
    ...
    public void step(Program program) {
        ...
        try {
            ...
            String hint = "";
            long callEnergon = 0, memWords = 0; // parameters for logging
            long energonCost = op.getTier().asInt();
            long energonBefore = program.getEnergonLong();
            int stepBefore = program.getPC();
            EnergonCost energonCosts = blockchainConfig.getEnergonCost();
            DataWord adjustedCallEnergon = null;
            ...
            // Calculate fees and spend Energon
            switch (op) {
                case STOP:
                    energonCost = energonCosts.getSTOP();
                    break;
                case SUICIDE:
                    energonCost = energonCosts.getSUICIDE();
                    DataWord suicideAddressWord = stack.get(stack.size() - 1);
                    if (!program.getStorage().isExist(suicideAddressWord.getLast20Bytes())) {
                        energonCost += energonCosts.getNEW_ACCT_SUICIDE();
                    }
                    break;
                case SSTORE:
                    ...
                    if (oldValue == null && !newValue.isZero())
                        energonCost = energonCosts.getSET_SSTORE();
                    else if (oldValue != null && newValue.isZero()) {
                        // todo: EnergonREFUND counter policy

                        // refund step cost policy.
                        program.futureRefundEnergon(energonCosts.getREFUND_SSTORE());
                        energonCost = energonCosts.getCLEAR_SSTORE();
                    } else
                        energonCost = energonCosts.getRESET_SSTORE();
                    break;
                case SLOAD:
                    energonCost = energonCosts.getSLOAD();
                    break;
                case BALANCE:
                    energonCost = energonCosts.getBALANCE();
                    break;

                // These all operate on memory and therefore potentially expand it:
                case MSTORE:
                    energonCost += calcMemEnergon(energonCosts, oldMemSize, memNeeded(stack.peek(), DataWord.of(32)), 0);
                    break;
                case MSTORE8:
                    energonCost += calcMemEnergon(energonCosts, oldMemSize, memNeeded(stack.peek(), DataWord.ONE), 0);
                    break;
                case MLOAD:
                    energonCost += calcMemEnergon(energonCosts, oldMemSize, memNeeded(stack.peek(), DataWord.of(32)), 0);
                    break;
                case RETURN:
                case REVERT:
                    energonCost = energonCosts.getSTOP() + calcMemEnergon(energonCosts, oldMemSize,
                            memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0);
                    break;
                case SHA3:
                    energonCost = energonCosts.getSHA3() + calcMemEnergon(energonCosts, oldMemSize, memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0);
                    DataWord size = stack.get(stack.size() - 2);
                    long chunkUsed = (size.longValueSafe() + 31) / 32;
                    energonCost += chunkUsed * energonCosts.getSHA3_WORD();
                    break;
                case CALLDATACOPY:
                case RETURNDATACOPY:
                    energonCost += calcMemEnergon(energonCosts, oldMemSize,
                ...
                case CODECOPY:
                    ...
                    break;
                case EXTCODESIZE:
                    energonCost = energonCosts.getEXT_CODE_SIZE();
                    break;
                case EXTCODECOPY:
                    energonCost = energonCosts.getEXT_CODE_COPY() + calcMemEnergon(energonCosts, oldMemSize,
                            memNeeded(stack.get(stack.size() - 2), stack.get(stack.size() - 4)),
                            stack.get(stack.size() - 4).longValueSafe());
                    break;
                case EXTCODEHASH:
                    energonCost = energonCosts.getEXT_CODE_HASH();
                    break;
                case CALL:
                case CALLCODE:
                case DELEGATECALL:
                case STATICCALL:
                    ...
                    energonCost = energonCosts.getCALL();
                    DataWord callEnergonWord = stack.get(stack.size() - 1);
                    ...
                    if (op == CALL) {
                        if (!program.getStorage().isExist(callAddressWord.getLast20Bytes())) {
                            energonCost += energonCosts.getNEW_ACCT_CALL();
                        }
                    }

                    //TODO #POC9 Make sure this is converted to BigInteger (256num support)
                    if (!value.isZero() )
                        energonCost += energonCosts.getVT_CALL();
                    ...
                    energonCost += calcMemEnergon(energonCosts, oldMemSize, in.max(out), 0);

                    if (energonCost > program.getEnergon().longValueSafe()) {
                        throw Program.Exception.notEnoughOpEnergon(op, callEnergonWord, program.getEnergon());
                    }

                    DataWord EnergonLeft = program.getEnergon();
                    DataWord subResult = EnergonLeft.sub(DataWord.of(energonCost));
                    adjustedCallEnergon = blockchainConfig.getCallEnergon(op, callEnergonWord, subResult);
                    energonCost += adjustedCallEnergon.longValueSafe();
                    break;
                case CREATE:
                    energonCost = energonCosts.getCREATE() + calcMemEnergon(energonCosts, oldMemSize,
                ...
                case LOG4:
                    ...
                    BigInteger dataCost = dataSize.multiply(BigInteger.valueOf(energonCosts.getLOG_DATA_ENERGON()));
                    if (program.getEnergon().value().compareTo(dataCost) < 0) {
                        throw Program.Exception.notEnoughOpEnergon(op, dataCost, program.getEnergon().value());
                    }

                    energonCost = energonCosts.getLOG_ENERGON() +
                            energonCosts.getLOG_TOPIC_ENERGON() * nTopics +
                            energonCosts.getLOG_DATA_ENERGON() * stack.get(stack.size() - 2).longValue() +
                            calcMemEnergon(energonCosts, oldMemSize, memNeeded(stack.peek(), stack.get(stack.size() - 2)), 0);
                    break;
                case EXP:

                    DataWord exp = stack.get(stack.size() - 2);
                    int bytesOccupied = exp.bytesOccupied();
                    energonCost = energonCosts.getEXP_ENERGON() + energonCosts.getEXP_BYTE_ENERGON() * bytesOccupied;
                    break;
                default:
                    break;
            }

            //DEBUG System.out.println(" OP IS " + op.name() + " EnergonCOST IS " + EnergonCost + " NUM IS " + op.asInt());
            program.spendEnergon(energonCost, op.name());

            // Log debugging line for VM
            if (program.getNumber().intValue() == dumpBlock)
                this.dumpLine(op, energonBefore, energonCost + callEnergon, memWords, program);
            ...
            switch (op) {
                ...
                break;
                case GASPRICE: {
                    DataWord EnergonPrice = program.getEnergonPrice();

                    if (logger.isInfoEnabled())
                        hint = "price: " + EnergonPrice.toString();

                    program.stackPush(EnergonPrice);
                    program.step();
                }
                break;
                ...
                break;
                case GASLIMIT: {
                    DataWord energonlimit = program.getEnergonLimit();

                    if (logger.isInfoEnabled())
                        hint = "energonlimit: " + energonlimit;

                    program.stackPush(energonlimit);
                    program.step();
                }
                break;
                case POP: {
                ...
                break;
                case GAS: {
                    DataWord energon = program.getEnergon();

                    if (logger.isInfoEnabled())
                        hint = "" + energon;

                    program.stackPush(energon);
                    program.step();
                }
                break;
                ...
                case DELEGATECALL:
                case STATICCALL: {
                    program.stackPop(); // use adjustedCallEnergon instead of requested
                    ...
                    if (!value.isZero()) {
                        adjustedCallEnergon = adjustedCallEnergon.add(DataWord.of(energonCosts.getSTIPEND_CALL()));
                    }
                    ...
                    if (logger.isInfoEnabled()) {
                        hint = "addr: " + toHexString(codeAddress.getLast20Bytes())
                                + " Energon: " + adjustedCallEnergon.shortHex()
                    ...
                    MessageCall msg = new MessageCall(
                            op, adjustedCallEnergon, codeAddress, value, inDataOffs, inDataSize,
            ...
            if (logger.isInfoEnabled() && !op.isCall())
                logger.info(logString, String.format("%5s", "[" + program.getPC() + "]"),
                        String.format("%-12s",
                                op.name()), program.getEnergon().value(),
                        program.getCallDeep(), hint);

            vmCounter++;
        } catch (RuntimeException e) {
            logger.warn("VM halted: [{}]", e);
            program.spendAllEnergon();
     ...
     *  - standard+ (owner address, program counter, operation, Energon left)
     *  - pretty (stack, memory, storage, level, contract,
     *              vmCounter, internalSteps, operation
                    EnergonBefore, EnergonCost, memWords)
     */
    private void dumpLine(OpCode op, long EnergonBefore, long EnergonCost, long memWords, Program program) {
        if (config.dumpStyle().equals("standard+")) {
            ...
            String EnergonString = toHexString(program.getEnergon().getNoLeadZeroesData());

            dumpLogger.trace("{} {} {} {}", addressString, pcString, opString, EnergonString);
